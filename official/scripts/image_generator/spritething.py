    ...
    for y in range(0, size):
        ...
        for x in range(0, size):
            ...
            if (element == int(size/2) or element == 0):
                i *= -1;
            element += i
def main(size, invaders, imgSize, quantity):
    for count,i in enumerate(range(0,quantity)):   
        ...
        origImage.save(f"Hackers/Hackers-+{str(size)}{x}{str(size)}-{str(invaders)}-{str(imgSize)}-{count}.jpg")
